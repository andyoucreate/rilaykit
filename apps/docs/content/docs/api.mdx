...
**Parameters**:
- `fieldId: string` -- The field ID to watch for changes
- `handler: FieldEffectHandler` -- Callback invoked when the watched field's value changes
...
type FieldEffects = readonly FieldEffect[];
```
...
validateSchema(schema, rilConfig, registry); // throws on error
```
