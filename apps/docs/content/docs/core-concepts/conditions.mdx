...
```typescript
const workflow = rilay.flow('user-onboarding')
  .addStep({
    id: 'personal-info',
...
// Real-world example
const workflow = rilay.flow('loan-application')
  .addStep({
    id: 'applicant-info',
...
```typescript
const checkoutForm = rilay.form('checkout')
  .add({
    id: 'billingAddress',
    type: 'address',
...
```typescript
const surveyForm = rilay.form('customer-survey')
  .add({
    id: 'satisfaction',
    type: 'select',
    props: {
...
```typescript
const jobApplicationFlow = rilay.flow('job-application')
  .addStep({
    id: 'basic-info',
