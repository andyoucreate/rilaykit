...
For complete production scenarios (SaaS onboarding, KYC verification, dynamic pricing), see the [Real-World Examples](/guides/real-world-examples) guide.
...
<Tabs items={['Component', 'Configuration', 'Usage']}>
  ...
  <Tab value="Usage">
    ...
    const profileForm = form.create(muiRilay, 'user-profile')
      .add({
        id: 'name',
        type: 'input',
...
<Tabs items={['Component', 'Configuration', 'Usage']}>
  ...
  <Tab value="Usage">
    ...
    const contactForm = form.create(shadcnRilay, 'contact')
      .add({
        id: 'name',
        type: 'input',
...
Build a multi-step user onboarding workflow using `flow.create(rilay, )`.
...
// Step 3: Confirmation
const confirmForm = form.create(rilay, 'confirm')
  .add({
    id: 'terms',
    type: 'checkbox',
...
// Build the workflow
const onboardingWorkflow = flow.create(rilay, 'onboarding', 'User Onboarding')
  .step({
    id: 'account',
