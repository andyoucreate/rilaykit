...
The `submit` function accepts either a `React.FormEvent` (for `<form onSubmit>`) or a `SubmitOptions` object for programmatic control:

```ts
interface SubmitOptions {
  force?: boolean;      // Skip validation, submit all values
  skipInvalid?: boolean; // Run validation, exclude invalid fields
}
```
...
await submit();                      // Standard: validate then submit
await submit({ force: true });       // Skip validation entirely
await submit({ skipInvalid: true }); // Submit only valid fields
```
