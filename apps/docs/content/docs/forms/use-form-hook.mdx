...
Import the hook from `@rilaykit/form-builder` and call it inside any component that is a child of a `FormProvider`.

```tsx
import { useFormContext } from '@rilaykit/form-builder';
...
- `formState`: An object containing the core state of the form.
  ...
  - `touched`: A `Set` of field IDs that the user has interacted with.
...
- `setError(fieldId, errors)`: Manually sets validation errors for a field.
- `clearError(fieldId)`: Clears errors for a specific field.
- `markFieldTouched(fieldId)`: Marks a field as "touched".
- `validateField(fieldId, value?)`: Triggers validation for a single field.
- `validateAllFields()`: Validates all fields in the form.
