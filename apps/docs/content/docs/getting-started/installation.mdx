---
title: Installation
description: Install RilayKit packages in your React project with step-by-step instructions.
---
import { Tab, Tabs } from 'fumadocs-ui/components/tabs';
import { Callout } from 'fumadocs-ui/components/callout';

RilayKit is modular by design. Install only the packages you need. `@rilaykit/core` is always required as the foundation.

## Quick Start

For most use cases, start with the core and forms packages:

<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab value="pnpm">
    ```bash
    pnpm add @rilaykit/core @rilaykit/forms
    ```
  </Tab>
  <Tab value="npm">
    ```bash
    npm install @rilaykit/core @rilaykit/forms
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @rilaykit/core @rilaykit/forms
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @rilaykit/core @rilaykit/forms
    ```
  </Tab>
</Tabs>

## Package Overview

| Package | License | Description | Use Case |
|---------|---------|-------------|----------|
| `@rilaykit/core` | MIT | Core engine, types, validation system | **Required** - Foundation for all RilayKit usage |
| `@rilaykit/forms` | MIT | Form builder and React components | Single-page forms, basic workflows |
| `@rilaykit/workflow` | Commercial | Multi-step workflows with advanced features | Complex onboarding, multi-page forms, analytics |
| `@rilaykit/validation-adapters` | MIT | ~~Zod, Yup, and Joi integrations~~ **[DEPRECATED]** Use Standard Schema instead | When using external validation libraries |

## Advanced Installation

### Multi-Step Workflows

For complex workflows with step navigation, persistence, and analytics:

<Callout type="info">
`@rilaykit/workflow` requires a commercial license for production use.
</Callout>

<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab value="pnpm">
    ```bash
    pnpm add @rilaykit/core @rilaykit/forms @rilaykit/workflow
    ```
  </Tab>
  <Tab value="npm">
    ```bash
    npm install @rilaykit/core @rilaykit/forms @rilaykit/workflow
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @rilaykit/core @rilaykit/forms @rilaykit/workflow
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @rilaykit/core @rilaykit/forms @rilaykit/workflow
    ```
  </Tab>
</Tabs>

### Validation Adapters

If you prefer using established validation libraries:

<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab value="pnpm">
    ```bash
    # For Zod users (recommended) - now with Standard Schema
    pnpm add zod
    
    # For Yup users - now with Standard Schema  
    pnpm add yup
    
    # For Joi users - now with Standard Schema
    pnpm add joi
    ```
  </Tab>
  <Tab value="npm">
    ```bash
    # For Zod users (recommended) - now with Standard Schema
    npm install zod
    
    # For Yup users - now with Standard Schema
    npm install yup
    
    # For Joi users - now with Standard Schema  
    npm install joi
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    # For Zod users (recommended) - now with Standard Schema
    yarn add zod
    
    # For Yup users - now with Standard Schema
    yarn add yup
    
    # For Joi users - now with Standard Schema
    yarn add joi
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    # For Zod users (recommended) - now with Standard Schema
    bun add zod
    
    # For Yup users - now with Standard Schema
    bun add yup
    
    # For Joi users - now with Standard Schema
    bun add joi
    ```
  </Tab>
</Tabs>

## System Requirements

<Callout type="warning">
RilayKit requires modern React and TypeScript versions for optimal type safety and performance.
</Callout>

### Required Dependencies

```json
{
  "peerDependencies": {
    "react": ">=18.0.0",
    "react-dom": ">=18.0.0"
  },
  "devDependencies": {
    "typescript": ">=5.0.0"
  }
}
```

### Framework Compatibility

RilayKit works with any React-based framework:

- ✅ **Next.js** (App Router & Pages Router)
- ✅ **Vite** + React
- ✅ **Create React App** (CRA)
- ✅ **Remix**
- ✅ **Gatsby**
- ✅ **Expo** (React Native compatible)

## Verification

Verify your installation by importing the core package:

```tsx title="test-installation.tsx"
import { ril } from '@rilaykit/core';
import { Form } from '@rilaykit/forms';

// If this imports without errors, you're ready to go!
console.log('RilayKit is ready!');
```

## Next Steps

- **New to RilayKit?** → Start with [Your First Form](/getting-started/your-first-form)
- **Want to see examples?** → Browse the [Examples Gallery](/examples)
- **Coming from another library?** → Check our [Migration Guides](/migration)
- **Enterprise setup?** → Review [Configuration Best Practices](/guides/configuration) 