...
Now, use the `rilay` instance you created to build a form. You call `.createForm()` on your instance, which gives you a form builder that is aware of all your registered components and renderers.
...
export const contactForm = rilay
  .createForm('contact-us') // Pass a unique ID for the form
  .addField({
    id: 'name',
    type: 'text',
...
Finally, use the `<Form>` component from `@rilaykit/forms` to render your form. You can pass the form builder instance directly to it—it will be built automatically.
