...
<Tabs items={['Components', 'Configuration', 'Form']}>
  ...
  <Tab value="Configuration">
    ...
    // Zod now supports Standard Schema natively
    import { z } from 'zod';
...
<Tabs items={['Components', 'Configuration', 'Form']}>
  ...
  <Tab value="Configuration">
    ...
    // Option 1: Using RilayKit built-in validators
    const contactFormBuiltIn = rilay
      ...
      .add({
        ...
        validation: { validate: [required('Email is required'), email('Invalid email')] }
      })
      .add({
        id: 'message',
        type: 'input',
        props: { placeholder: 'Message', multiline: true },
        validation: { validate: minLength(10, 'Message too short') }
      });

    // Option 2: Using Zod directly (no adapters needed!)
    const contactFormZod = rilay
      .form('contact')
      .add({
        id: 'name',
        type: 'input',
        props: { placeholder: 'Name' },
        validation: { validate: z.string().min(1, 'Name is required') }
      })
      .add({
        id: 'email',
        type: 'input',
        props: { placeholder: 'Email' },
        validation: { validate: z.string().email('Invalid email') }
      })
      .add({
        id: 'message',
        type: 'input',
        props: { placeholder: 'Message', multiline: true },
        validation: { validate: z.string().min(10, 'Message too short') }
      });

    // Option 3: Using Yup directly (no adapters needed!)  
    const contactFormYup = rilay
      ...
      .add({
        ...
        type: 'input',
        props: { placeholder: 'Name' },
        validation: { validate: yup.string().required('Name is required') }
      })
      .add({
        id: 'email',
        type: 'input',
        props: { placeholder: 'Email' },
        validation: { validate: yup.string().email('Invalid email').required() }
      });

    // Option 4: Mix different libraries!
    const contactFormMixed = rilay
      .form('contact')
      .add({
        id: 'email',
        type: 'input',
    ...
    export { contactFormBuiltIn, contactFormZod, contactFormYup, contactFormMixed };
    ```
  </Tab>
  <Tab value="Form">
