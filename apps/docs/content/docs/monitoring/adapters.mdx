...
const adapter = new LocalStorageMonitoringAdapter(1000);
monitor.addAdapter(adapter);
```
...
<Callout>
`LocalStorageMonitoringAdapter` is intended for buffering or debugging. For long-term persistence, forward events to a remote endpoint using `RemoteAdapter`.
</Callout>
...
<Tabs items={['Development', 'Staging', 'Production']}>
  ...
  <Tab value="Staging">
    Combine `ConsoleAdapter` for visibility with `LocalStorageMonitoringAdapter` for persistence across refreshes.

    ```ts
    monitor.addAdapter(new ConsoleAdapter('debug'));
    monitor.addAdapter(new LocalStorageMonitoringAdapter(5000));
    ```
  </Tab>
  <Tab value="Production">
    Use `RemoteAdapter` to send events to your analytics backend. Optionally add `LocalStorageMonitoringAdapter` as a fallback buffer.
...
This is useful for scenarios like enabling verbose logging when a user opens a debug panel, or temporarily attaching a `LocalStorageMonitoringAdapter` to capture a reproduction of a bug.
