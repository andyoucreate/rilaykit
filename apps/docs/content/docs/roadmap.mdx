...
| Server-driven forms with `fromSchema()` — JSON to FormConfig | `@rilaykit/forms` | Shipped |
| Schema validation (`validateSchema`) and type guard (`isFormSchema`) | `@rilaykit/forms` | Shipped |
| Schema registry for custom validators and effect handlers | `@rilaykit/forms` | Shipped |
| Flat and row-based schema layouts with repeatable support | `@rilaykit/forms` | Shipped |
