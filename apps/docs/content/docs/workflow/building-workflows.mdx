...
const accountForm = formBuilder
  .addFields([
    { id: 'email', type: 'email', props: { label: 'Email' }, validation: (v) => v.string().email() },
    { id: 'password', type: 'password', props: { label: 'Password' }, validation: (v) => v.string().min(8) },
  ]);

const profileForm = formBuilder
  .addRowFields([
    { id: 'firstName', type: 'text', props: { label: 'First Name' } },
    { id: 'lastName', type: 'text', props: { label: 'Last Name' } },
  ]);

const confirmationForm = formBuilder
  .addField({
    id: 'terms',
    ...
    validation: (v) => v.boolean().refine(val => val === true, { message: 'You must accept the terms' }),
  });
```

Now, we use the `createFlow` builder to chain them together into a workflow:
