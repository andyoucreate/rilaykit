...
- `workflowState`: An object containing the raw state of the workflow.
  ...
  - `errors`: Validation errors for the current step's fields.
...
- `saveDraft()`: Manually saves the current workflow state if persistence is enabled.
- `loadDraft()`: Manually loads a saved workflow state.
- `clearDraft()`: Clears a saved workflow state.
