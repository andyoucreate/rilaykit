...
- **Phase proof gate (mandatory, every phase).** Per-task TDD proves units; each phase ends with a dedicated proof task that proves FEATURES: (1) a **feature proof matrix** enumerating every user-facing capability of the phase with the exact test (`file:testname`) that fails if it breaks — a row without a proving test is a test to write; the filled matrix is committed as the phase's proof record; (2) **hardening tests** for the scenarios unit tests miss (double-submit, all-hidden forms, min/max bounds, back-navigation data retention, live predicate flips, hidden-step index mapping — and for P3: partial-JSON chunk simulations, HITL round-trips, adapter contract tests against recorded provider fixtures); (3) the **coverage gate** at the repo's configured thresholds (90/85/90/90 — never lowered to pass); (4) an **adversarial checker pass** over the whole phase diff (tests-prove-behavior / DRY / elegance / conventions lenses) with every confirmed finding fixed. P1's gate is Task 19 of its plan; P2 and P3 plans MUST include the equivalent final task.
