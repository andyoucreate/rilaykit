...
export class FormBuilder {
  ...
  addRowFields(
    ...
    for (const fieldConfig of fieldConfigs) {
      const components = this.config.getComponentsBySubType(
        fieldConfig.componentSubType
      );

      if (components.length === 0) {
        throw new Error(
          `No component found with subType "${fieldConfig.componentSubType}"`
        );
      }
  ...
  updateField(
    fieldId: string,
    updates: Partial<Omit<FormFieldConfig, "id">>
  ): this {
  ...
  getStats(): {
    ...
    return {
      ...
      averageFieldsPerRow:
        this.rows.length > 0 ? allFields.length / this.rows.length : 0,
