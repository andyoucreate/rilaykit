...
describe('Workflow Conditions', () => {
  ...
  beforeEach(() => {
    ...
    basicForm = form.create(rilConfig, 'basic-form')
      .add({
        type: 'text',
  ...
  test('should add conditions to step during creation', () => {
    const workflow = flow.create(rilConfig, 'test-workflow', 'Test Workflow')
      .addStep({
        id: 'personalInfo',
  ...
  test('should handle complex step conditional logic', () => {
    const workflow = flow.create(rilConfig, 'test-workflow', 'Test Workflow')
      ...
      .addStep({
        ...
        formConfig: basicForm,
        conditions: {
          visible: when('userInfo.age').greaterThan(18)
